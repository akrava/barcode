...
const BARCODE_DIGITS_COUNT = 13;
const BARCODE_NAME = "ITF-14";

type EncodeState = {
    barcode_source_number_val: string,
    barcode_source_number_is_valid: boolean,
    barcode_obj: ITF14 | null,
...
class Encode extends React.Component {
    ...
    check_if_barcode_source_number_val_is_valid(val: string) {
        return val.length == BARCODE_DIGITS_COUNT && /^[0-9]{13,}$/.test(val);
    }
            ...
            return (
                <div style={{ textAlign: "center",}}>
                    ...
                    <div style={{ display: "flex", justifyContent: "center", alignItems: "center", flexDirection: "column", marginBottom: "20px" }}>
                        <canvas width="300px" height="100px" id="barcode-canva" style={{width: "300px", height: "100px", marginBottom: "10px"}}></canvas>
                    ...
                    <textarea readOnly={true} value={this.state.barcode_obj.getEncodedData()} style={{ fontFamily: "monospace", width: "920px", height: "30px", resize: "none", display: "block", marginLeft: "auto", marginRight: "auto" }}></textarea>
    ...
    componentDidUpdate() {
        if (this.state.barcode_source_number_is_valid) {
            ...
            if (canvas && canvas.getContext) {
                ...
                const barcode_arr = this.state.barcode_obj.getEncodedDataArr();
                ...
                const height = 70;
                ctx.fillStyle = "white";
                ctx.fillRect(0, 0, 275, 100);
                ctx.fillStyle = "black";

                for (let i = 0; i < barcode_arr.length; i++) {
                    if (barcode_arr[i] == 1) {
                        ctx.fillRect(i*i_width, 0, i_width, height);
                    }
                }

                const check_digit = this.state.barcode_obj.getCheckDigit();
                const barcode = this.state.barcode_obj.sourceNumber() + check_digit.toString();
                ctx.font = "20px Arial";

                ctx.fillText(barcode[0], 2+ 0, 5+ 88);

                for (let i = 1; i < 14; i++) {
                    ctx.fillText(barcode[i], 2+ 7 + i * 19, 5+ 88);
                }
    ...
    saveToFile = () => {
        const canvas = document.getElementById('barcode-canva') as HTMLCanvasElement;
        canvas.toBlob((blob) => {
            saveAs(blob, "barcode_" + this.state.barcode_obj.sourceNumber() + ".png");
        });
    }

    saveToFileAsText = () => {
        saveAs(new Blob([this.state.barcode_obj.getEncodedData()], {type: "text/plain;charset=utf-8"}), "barcode_" + this.state.barcode_obj.sourceNumber() + ".txt", { autoBom: true });
    }
        ...
        return (
            <>
                ...
                <p className="source-barcode-actions">
                    <button className="btn btn-primary" onClick={this.reset_barcode_source_number_val}>Reset barcode input</button>
                </p>
            </>
        );
    }
    ...
    handle_set_product = (val: string) => {
        ...
        if (idx >= 0) {
            const t5 =  new ITF14(this.state.products[idx].code.toString())
            this.setState({
                barcode_source_number_val: t5.sourceNumber(),
